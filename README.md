...
| # | 作品 | 类型 | 一句话 | 深度 |
|---|------|------|--------|----------|
| 1 | [墨 · INK](ink/index.html) | 视觉实验 / 流体 | 水墨流体画室：缓行则浓，疾书则淡 | **四笔刷**（圆笔/硬笔出锋/泼墨/水笔破墨）· **八色墨可自定**（长按色点弹取色器，LUT 色带驻点实时改写，混色随新色带即刻生效）· **三纸质**（阻力/涡度/晕染各异，纸面吃笔有涩顺）· **纸面检视**（滚轮缩放/拖移）· **湿边与积墨**（dye 第三通道记录湿度：落笔后墨随水缘聚集、浓墨处颜料随纸纹沉淀析出，干后定格）· **提按笔锋**（半径随压感与速度起伏，起笔尖锋入纸）· **枯笔丝缕**（飞白沿运笔方向连贯成丝，不再是噪点）· **悬停滴墨**（按住不动墨滴积聚洇开；水笔悬停持续化墨）· 细笔宽至发丝 · 8 步撤销 · 压感/倾斜 · **动态三档画质** · IndexedDB 自动存档（分辨率切换后载入不再错位）· **画册长卷导出** · **题款装裱**（干支年+朱印+画框）· **印面图片上传** |
...
- **回声**：方向键/WASD 移动 · 空格/点击发声呐 · `#lv0`~`#lvN`（N≥12 无尽）· `#edit[=码]` 造井 · `#dbg=daily` 每日
- **造境**：`1-6` 预设 · `0` 每日气象 · `T` 主题 · `M` 心境 · `F` 时光流转 · `R` 录制 · `←→` 时刻 · `#w=<base64>` 直达场景
