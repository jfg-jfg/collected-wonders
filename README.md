...
| 5 | [褶皱 · FOLD](fold/index.html) | 技术 / 分形 | 一颗无限细节的星球 | **v2 两遍引擎**（迭代数据一次写入 16F FBO，色相/调色板切换零重算）· **精度分层**（浅层单精度提速约 3 倍 / 深层修复版 df64 至 10¹³）· **DE 褶皱墨线**（复导数距离估计等距线）· 条纹平均织物着色 · **地质层调色**（深度=地层，越深越近单色冷墨）· **纪元分层命名**（浅海/尘埃/洇墨/灯下，量化格稳定不闪烁）· **深度手记徽章**（1e3/1e6/1e9/1e12，◆◇ HUD）· 三模式（曼/儒/舰）· 密度巡航 · 待机深潜 · PNG 存图 · 坐标分享/书签 |
