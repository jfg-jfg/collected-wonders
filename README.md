...
| 4 | [纸间 · LETTERS](letters/index.html) | 叙事 / 书信 | 五个信箱，或让 AI 写、亲手写你的故事 | **五个内置故事**（守灯人 12 信 4 结局·隐藏结局 / 雪国列车员 8 信 4 结局·楷体蓝笺 / 深海电台 8 信 4 结局·暗纸青字 / 停钟巷 8 信 4 结局·隶书铜笺·隐藏结局 / **观星台 8 信 5 结局·仿宋星蓝笺·隐藏「灯还亮着」**——27 年彗星周期双线通信，与褶皱触底句互文）· **隐藏结局引擎**（require 条件满足时替换常规结局）· per-story 字体/纸色/做旧(age)/明信片(postcard) · WebAudio 纸声（拆封/展信/轻叩/结局钟）· **信堆重读** · **AI 参数化创作**（题材/信数/结局数/笔调/通信关系 → 提示词）· **✎ 手写编辑器**（节点式回信分支/四纸笔预设+自定纸墨印三色/**隐藏结局可视化配置**（勾选触发信+替换目标）/试读/改编任意故事——含导入包，隐藏与配色随稿往返/导出）· **导入校验**（结构/悬挂引用/文件读取）· 分享（直达链接/JSON） |
