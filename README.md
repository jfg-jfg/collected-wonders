...
| 5 | [褶皱 · FOLD](fold/index.html) | 技术 / 分形 | 一颗无限细节的星球 | **三模式**（曼德博/儒利亚/燃烧之舰）· df64 双精度模拟（Dekker/Knuth 双单算术），缩放至 10¹³ · **密度巡航**（readPixels 8×6 网格方差分析，飞向图案最繁密处）· 四调色板 · 色相缓移流彩 · 深度玄名（按坐标哈希生成）· 平滑惯性缩放 · 待机深潜 · PNG 存图 · 坐标分享/书签 |
...
- 冒烟测试：`bash tools/smoke-test.sh`（六站 JS 错误检查；`--dump-dom` 会执行 JS，页面将错误写入 DOM 供捕获）
- 关卡验证：`node tools/validate-levels.mjs`（回声九关 + 40 样本随机压力）
